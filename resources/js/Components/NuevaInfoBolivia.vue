<template>
    <section id="categorias" class="bg-[#0D6734] py-20 px-6 md:px-12 lg:px-20">
      <div class="max-w-6xl mx-auto">
        <div class="inline-block mb-12 text-center w-full" data-aos="fade-up" data-aos-duration="1000">
          <h2 class="text-3xl md:text-4xl font-extrabold mb-3 leading-tight text-[#FBF8F3]" data-aos="fade-up" data-aos-duration="1200">
            Impulsando Emprendimientos con Valor Amazónico
          </h2>
          <div class="h-1 w-24 bg-[#AB8219] rounded-full mx-auto" data-aos="fade-up" data-aos-duration="1400"></div>
          <p class="text-[#FBF8F3] mt-4 max-w-2xl mx-auto" data-aos="fade-up" data-aos-duration="1600">
            Se mapearon 25 iniciativas y, tras un proceso de evaluación, 11 fueron seleccionadas para recibir acompañamiento estratégico y visibilización. La clasificación se basó en el ciclo de una planta, representando las etapas de germinación, crecimiento, florecimiento y maduración. Así reconocemos el proceso único de cada emprendimiento, que combina tradición, sostenibilidad y visión de futuro.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6" data-aos="fade-up" data-aos-duration="1800">
          <div v-for="(categoria, index) in categoriasFiltradas" :key="categoria.nombre"
            @click="toggleFlip(index)" :class="[
              'flip-card group relative rounded-xl transition-all duration-300 border border-[#0D6734]',
