  ...
  <section id="noticias" class="bg-white py-16 px-4 overflow-hidden">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-16 text-center" data-aos="fade" data-aos-duration="1000">
        <h2 class="text-4xl md:text-5xl font-extrabold text-[#124228] relative inline-block" data-aos="fade" data-aos-duration="1200">
          <span class="relative z-10">Noticias</span>
          <div class="h-1 w-24 bg-[#73A847] rounded-full mx-auto mb-6" data-aos="fade" data-aos-duration="1400"></div>
        </h2>
        <p class="text-lg text-gray-600 mt-6 max-w-3xl mx-auto" data-aos="fade" data-aos-duration="1600">
          Mantente informado con las últimas actualizaciones, eventos y logros relacionados con la conservación y las comunidades de la Amazonía en Ecuador.
        </p>
      </div>
      ...
      <div class="space-y-20 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar" data-aos="fade" data-aos-duration="1000">
        <div v-for="(noticia, index) in noticias" :key="noticia.id" class="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
          ...
          <div :class="index % 2 === 0 ? 'order-2 md:order-1' : 'order-1 md:order-2'">
            <h2 class="text-3xl font-semibold text-[#124228] mb-4" data-aos="fade" data-aos-duration="1200">
              {{ noticia.titulo }}
            </h2>
...
onMounted(async () => {
  try {
    const { data } = await axios.get('/api/noticias-ecuador')
    noticias.value = data
    console.log('Noticias Ecuador cargadas:', data)
  } catch (error) {
    console.error('Error cargando noticias de Ecuador:', error)
  }
})
...
.custom-scrollbar::-webkit-scrollbar-thumb {
    ...
    border-radius: 9999px;
    border: 2px solid rgb(249, 236, 191);
}
