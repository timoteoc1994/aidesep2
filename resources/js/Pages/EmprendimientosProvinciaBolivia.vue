<template>
  ...
  <meta
    name="description"
    content="Conoce los emprendimientos sostenibles en la provincia de {{ categoriaNormalizada }} que integran conocimientos ancestrales, innovación y compromiso ambiental."
  />
  <meta
    name="keywords"
    content="Amazon Fair Trade, emprendimientos, sostenibilidad, {{ categoriaNormalizada }}, economía indígena, Amazonía boliviana"
  />
  ...
  <meta
    ...
    content="Explora iniciativas en {{ categoriaNormalizada }} que fortalecen la identidad amazónica a través de prácticas responsables y saberes tradicionales."
  />
  <meta property="og:type" content="website" />
  <meta
    property="og:url"
    content="https://amazonfairtrade.org/emprendimientosProvinciaBolivia/{{ props.codigo }}"
  />
  <meta
    property="og:image"
    content="https://amazonfairtrade.org/images/amazonFairTrade/FondoIA.webp"
  />
  ...
  <meta
    ...
    content="Descubre los emprendimientos de {{ categoriaNormalizada }} impulsados por comunidades amazónicas con enfoque sostenible."
  />
  <meta
    name="twitter:image"
    content="https://amazonfairtrade.org/images/amazonFairTrade/FondoIA.webp"
  ...
  <link
    rel="canonical"
    href="https://amazonfairtrade.org/emprendimientosProvinciaBolivia/{{ props.codigo }}"
  />
</Head>

    <div v-if="cargando" class="fixed inset-0 bg-[#0D6734] z-50 flex flex-col items-center justify-center h-screen">
    ...
    <section class="py-16 px-6 md:px-12 lg:px-20 bg-[#0D6734] h-full">
        <div class="w-full mx-auto">
            ...
            <div class="flex flex-col md:flex-row gap-8">
                ...
                <div class="w-full sm:w-52 lg:w-56 shrink-0 hidden md:block">
                    <div class="bg-[#FFF8EA] p-4 rounded-lg sticky top-4 border-2 border-[#AB8219]">
                        <div class="mb-4 flex justify-between items-center">
                            ...
                            <button
                                ...
                                @click="resetearFiltros" class="text-xs text-[#AB8219] hover:text-[#374151]">
                                Resetear filtros
                            </button>
                        </div>
                            ...
                            <div class="mb-4 border-b border-[#AB8219] pb-3">
                                ...
                                <div v-if="mostrarSeccionPais"
                                    ...
                                    <div v-for="pais in paisesDisponibles" :key="pais" class="flex items-center">
                                        ...
                                        <label :for="`pais-${pais}`" class="ml-2 text-sm text-[#374151]">{{ pais
                                        }}</label>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div v-if="mostrarSeccionProvincia"
                                    ...
                                    <div v-for="provincia in provinciasDisponibles" :key="provincia"
                                        ...
                                        <input :id="`provincia-${provincia}`" type="checkbox" :value="provincia"
                                            ...
                                            class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                        <label :for="`provincia-${provincia}`" class="ml-2 text-sm text-[#374151]">{{
                                            provincia }}</label>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div v-if="mostrarSeccionEtiquetas"
                                    ...
                                    <div v-for="etiqueta in etiquetasDisponibles" :key="etiqueta"
                                        ...
                                        <input :id="`etiqueta-${etiqueta}`" type="checkbox" :value="etiqueta"
                                            ...
                                            class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                        <label :for="`etiqueta-${etiqueta}`" class="ml-2 text-sm text-[#374151]">{{
                                            etiqueta }}</label>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div v-if="mostrarSeccionCertificaciones"
                                    ...
                                    <div v-for="cert in certificacionesDisponibles" :key="cert"
                                        ...
                                        <input :id="`cert-${cert}`" type="checkbox" :value="cert"
                                            ...
                                            class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                        <label :for="`cert-${cert}`" class="ml-2 text-sm text-[#374151]">{{ cert
                                        }}</label>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="relative mb-6 flex items-center gap-2">
                        <div class="relative flex-1 group">
                            ...
                            <input v-model="busqueda" @input="handleBusqueda" type="text"
                                class="w-full pl-12 text-[#374151] font-bold border-2 border-[#AB8219] rounded-lg focus:outline-none focus:ring-0 focus:border-[#AB8219]"
                                placeholder="Buscar por nombre o descripción..." />
                        </div>

                        <!-- Botón filtros -->
                        <button @click="mostrarSidebarMovil = true"
                            class="md:hidden p-2 border-2 border-[#AB8219] rounded-lg text-[#FFF8EA] hover:bg-[#FFF8EA] hover:text-[#AB8219]">
                    ...
                    <div v-if="emprendimientos.length === 0" class="text-center py-16">
                        ...
                        <button @click="resetearFiltros"
                            class="mt-4 px-4 py-2 bg-[#AB8219] text-[#FFF8EA] hover:text-[#AB8219] font-medium rounded-lg hover:bg-[#FFF8EA] transition-all">
                            Limpiar filtros
                        </button>
                    </div>
                    ...
                    <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                        <div v-for="(e, index) in emprendimientos" :key="e.id" @click="verPerfilEmprendimiento(e.slug)"
                            ...
                            <div class="p-6">
                                ...
                                <p class="text-[#374151] mb-4 line-clamp-3">{{ e.descripcion }}</p>
            ...
            <div v-if="paginacion.last_page > 1" class="flex justify-center mt-8">
                <nav class="flex items-center gap-1 sm:gap-2" aria-label="Paginación">
                    ...
                    <button :disabled="paginacion.current_page === 1"
                        ...
                        class="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full border border-[#FFF8EA] text-[#FFF8EA] hover:bg-[#FFF8EA] hover:text-[#374151] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        aria-label="Anterior">
                        «
                    </button>
                    ...
                    <template v-for="page in totalPagesToShow" :key="page">
                        <button v-if="page !== '...'" @click="obtenerEmprendimientos(page)" :class="[
                            ...
                            page === paginacion.current_page
                                ? 'bg-[#FFF8EA] text-[#374151] border-[#FFF8EA] shadow-md'
                                : 'text-[#FFF8EA] border-[#FFF8EA] hover:bg-[#FFF8EA] hover:text-[#374151] transition-all'
            ...
            <div v-if="mostrarSidebarMovil"
                ...
                <div class="w-4/5 sm:w-2/3 h-full bg-white shadow-lg p-4 overflow-y-auto custom-scrollbar">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button @click="mostrarSidebarMovil = false" class="text-emerald-500 hover:text-[#AB8219]">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div>
                        ...
                        <div class="mb-4 flex justify-between items-center">
                            ...
                            <button
                                ...
                                @click="resetearFiltros" class="text-xs text-emerald-400 hover:text-[#AB8219]">
                                Resetear filtros
                            </button>
                        </div>
                            ...
                            <div v-if="mostrarSeccionPais"
                                ...
                                <div v-for="pais in paisesDisponibles" :key="pais" class="flex items-center">
                                    <input :id="`movil-pais-${pais}`" type="checkbox" :value="pais"
                                        v-model="filtrosPais"
                                        class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                    <label :for="`movil-pais-${pais}`" class="ml-2 text-sm text-[#374151]">{{ pais
                                    }}</label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div v-if="mostrarSeccionProvincia"
                                ...
                                <div v-for="provincia in provinciasDisponibles" :key="provincia"
                                    ...
                                    <input :id="`movil-provincia-${provincia}`" type="checkbox" :value="provincia"
                                        v-model="filtrosProvincia"
                                        class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                    <label :for="`movil-provincia-${provincia}`" class="ml-2 text-sm text-[#374151]">{{
                                        provincia }}</label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div v-if="mostrarSeccionEtiquetas"
                                ...
                                <div v-for="etiqueta in etiquetasDisponibles" :key="etiqueta" class="flex items-center">
                                    <input :id="`movil-etiqueta-${etiqueta}`" type="checkbox" :value="etiqueta"
                                        v-model="filtrosEtiquetas"
                                        class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                    <label :for="`movil-etiqueta-${etiqueta}`" class="ml-2 text-sm text-[#374151]">{{
                                        etiqueta }}</label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div v-if="mostrarSeccionCertificaciones"
                                ...
                                <div v-for="cert in certificacionesDisponibles" :key="cert" class="flex items-center">
                                    <input :id="`movil-cert-${cert}`" type="checkbox" :value="cert"
                                        v-model="filtroCertificaciones"
                                        class="w-4 h-4 text-[#AB8219] rounded focus:ring-0 focus:ring-offset-0" />
                                    <label :for="`movil-cert-${cert}`" class="ml-2 text-sm text-[#374151]">{{ cert
                                    }}</label>
                                </div>
                            </div>
                        </div>
...
const certificacionesDisponibles = ref([]);
...
// Computed para normalizar el nombre
const categoriaNormalizada = computed(() => {
    return provinciasBolivia[props.codigo] || 'Provincia desconocida';
});

// Función para convertir etiquetas en array
...
const cargarFiltros = async () => {
    try {
        const { data } = await axios.get(`/api/filtros/provinciaBolivia/${props.codigo}`);
        paisesDisponibles.value = data.paises || [];
        provinciasDisponibles.value = data.provincias || [];
        etiquetasDisponibles.value = data.etiquetas || [];
        certificacionesDisponibles.value = data.certificaciones || [];
    } catch (error) {
        console.error('❌ Error al cargar filtros:', error);
    }
};

// Cargar emprendimientos paginados
const obtenerEmprendimientos = async (pagina = paginacion.value.current_page) => {
    cargando.value = true;
    try {
        const params = {
            page: pagina,
            busqueda: busqueda.value || undefined,
            paises: filtrosPais.value.join(',') || undefined,
            provincias: filtrosProvincia.value.join(',') || undefined,
            etiquetas: filtrosEtiquetas.value.join(',') || undefined,
            certificaciones: filtroCertificaciones.value.join(',') || undefined,
        };

        const { data } = await axios.get(`/api/emprendimientos/provinciaBolivia/${props.codigo}`, { params });
...
// Watch para filtros y búsqueda
watch([busqueda, filtrosPais, filtrosProvincia, filtrosEtiquetas, filtroCertificaciones], () => {
    handleBusqueda();
});
