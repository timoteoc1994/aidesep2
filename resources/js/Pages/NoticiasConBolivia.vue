<template>
    <section id="noticias" class="bg-[#FBF8F3] py-16 px-4 overflow-hidden">
      <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-16 text-center" data-aos="fade" data-aos-duration="1000">
          ...
          <p class="text-lg text-[#244041] mt-6 max-w-3xl mx-auto" data-aos="fade" data-aos-duration="1600">
            Mantente informado con las últimas actualizaciones, eventos y logros relacionados con la conservación y las comunidades de la Amazonía en Bolivia.
          </p>
        </div>
        ...
        <div class="space-y-20 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar" data-aos="fade" data-aos-duration="1000">
          <div v-for="(noticia, index) in noticias" :key="noticia.id" class="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
            ...
            >
              ...
              />
              <div class="p-4">
                <h4 class="text-lg font-semibold text-[#FBF8F3] mb-2">{{ noticia.titulo }}</h4>
                <div class="flex items-center text-sm text-[#FBF8F3] mb-1">
                  <Calendar class="w-4 h-4 mr-2" />
                  {{ formatFecha(noticia.fecha) }} &nbsp; | &nbsp; {{ noticia.autor ?? 'Autor desconocido' }}
                </div>
                <div class="flex items-center text-sm text-[#FBF8F3]">
  ...
  onMounted(async () => {
    try {
      const { data } = await axios.get('/api/noticias-bolivia')
      noticias.value = data
      console.log('Noticias Bolivia cargadas:', data)
    } catch (error) {
      console.error('Error cargando noticias de Bolivia:', error)
    }
  })
